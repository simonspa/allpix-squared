...
ElectricFieldReaderModule::ElectricFieldReaderModule(
    Configuration &config, Messenger *, std::shared_ptr<Detector> detector)
...
void ElectricFieldReaderModule::init() {
  ...
  if ((field_model == "constant" || field_model == "linear") &&
    ...
    LOG(WARNING) << "Very high bias voltage of "
                 << Units::display(config_.get<double>("bias_voltage"), "kV")
  ...
  if (config_.count({"depletion_voltage", "depletion_depth"}) > 1) {
    throw InvalidCombinationError(
        config_, {"depletion_voltage", "depletion_depth"},
        "Depletion voltage and depth are mutually exclusive.");
  }

  // Set depletion depth to full sensor:
  auto model = detector_->getModel();
  auto depletion_depth =
      config_.get<double>("depletion_depth", model->getSensorSize().z());
  if (depletion_depth - model->getSensorSize().z() > 1e-9) {
    throw InvalidValueError(
        config_, "depletion_depth",
        "depletion depth can not be larger than the sensor thickness");
  }

  // Calculate thickness domain
  auto sensor_max_z =
      model->getSensorCenter().z() + model->getSensorSize().z() / 2.0;
  auto thickness_domain =
      std::make_pair(sensor_max_z - depletion_depth, sensor_max_z);
  ...
  if (field_model == "init") {
    ...
    detector_->setElectricFieldGrid(field_data.first, field_data.second,
                                    thickness_domain);
  } else if (field_model == "constant") {
    LOG(TRACE) << "Adding constant electric field";
    type = ElectricFieldType::CONSTANT;

    auto field_z = config_.get<double>("bias_voltage") /
                   getDetector()->getModel()->getSensorSize().z();
    LOG(INFO) << "Set constant electric field with magnitude "
              << Units::display(field_z, {"V/um", "V/mm"});
    ...
    auto depletion_voltage = config_.get<double>(
        "depletion_voltage", config_.get<double>("bias_voltage"));

    LOG(INFO) << "Setting linear electric field from "
              << Units::display(config_.get<double>("bias_voltage"), "V")
              << " bias voltage and " << Units::display(depletion_voltage, "V")
              << " depletion voltage";
    ElectricFieldFunction function =
        get_linear_field_function(depletion_voltage, thickness_domain);
    detector_->setElectricFieldFunction(function, thickness_domain, type);
  } else {
    throw InvalidValueError(config_, "model",
                            "model should be 'linear', 'constant' or 'init'");
  }
...
ElectricFieldFunction ElectricFieldReaderModule::get_linear_field_function(
    double depletion_voltage, std::pair<double, double> thickness_domain) {
  ...
  // Reduce the effective thickness of the sensor if voltage is below full
  // depletion
  if (bias_voltage < depletion_voltage) {
    eff_thickness *= sqrt(bias_voltage / depletion_voltage);
    depletion_voltage = bias_voltage;
  }
  LOG(TRACE) << "Effective thickness of the electric field: "
             << Units::display(eff_thickness, {"um", "mm"});
  return [bias_voltage, depletion_voltage, direction, eff_thickness,
          thickness_domain](const ROOT::Math::XYZPoint &pos) {
    double z_rel = thickness_domain.second - pos.z();
    double field_z =
        std::max(0.0, (bias_voltage - depletion_voltage) / eff_thickness +
                          2 * (depletion_voltage / eff_thickness) *
                              (1 - z_rel / eff_thickness));
 ...
 */
ElectricFieldReaderModule::FieldData
ElectricFieldReaderModule::read_init_field() {
  try {
    LOG(TRACE) << "Fetching electric field from init file";

    // Get field from file
    auto field_data =
        get_by_file_name(config_.getPath("file_name", true), *detector_.get());
    LOG(INFO) << "Set electric field with " << field_data.second.at(0) << "x"
              << field_data.second.at(1) << "x" << field_data.second.at(2)
              << " cells";
...
void ElectricFieldReaderModule::create_output_plots() {
  ...
  if (project != 'x' && project != 'y' && project != 'z') {
    throw InvalidValueError(config_, "output_plots_project",
                            "can only project on x, y or z axis");
  }

  auto model = detector_->getModel();
  if (config_.get<bool>("output_plots_single_pixel", true)) {
    // If we need to plot a single pixel we change the model to fake the sensor
    ...
    model->setNPixels(
        ROOT::Math::DisplacementVector2D<ROOT::Math::Cartesian2D<int>>(1, 1));
  }
  ...
  // Create 2D histogram
  auto histogram = new TH2F("field_magnitude", "electric field magnitude",
                            static_cast<int>(steps), min1, max1,
                            static_cast<int>(steps), min2, max2);
  histogram->SetMinimum(0);

  // Create 1D histogram
  auto histogram1D = new TH1F(
      "field1d_z", "electric field (z-component);z (mm);field strength (V/cm)",
      static_cast<int>(steps), min2, max2);
  ...
  if (project == 'x') {
    x = model->getSensorCenter().x() - model->getSensorSize().x() / 2.0 +
        config_.get<double>("output_plots_projection_percentage", 0.5) *
            model->getSensorSize().x();
  } else if (project == 'y') {
    y = model->getSensorCenter().y() - model->getSensorSize().y() / 2.0 +
        config_.get<double>("output_plots_projection_percentage", 0.5) *
            model->getSensorSize().y();
  } else {
    z = z_min +
        config_.get<double>("output_plots_projection_percentage", 0.5) *
            (z_max - z_min);
  ...
  for (size_t j = 0; j < steps; ++j) {
    if (project == 'x') {
      y = model->getSensorCenter().y() - model->getSensorSize().y() / 2.0 +
          ((static_cast<double>(j) + 0.5) / static_cast<double>(steps)) *
              model->getSensorSize().y();
    ...
    for (size_t k = 0; k < steps; ++k) {
      if (project == 'x') {
        z = z_min +
            ((static_cast<double>(k) + 0.5) / static_cast<double>(steps)) *
                (z_max - z_min);
 ...
 */
inline static void check_detector_match(Detector &detector, double thickness,
                                        double xpixsz, double ypixsz) {
  auto model = detector.getModel();
  // Do a several checks with the detector model
  if (model != nullptr) {
    if (std::fabs(thickness - model->getSensorSize().z()) >
        std::numeric_limits<double>::epsilon()) {
      LOG(WARNING) << "Thickness of sensor in file is "
                   << Units::display(thickness, "um")
                   << " but in the model it is "
                   << Units::display(model->getSensorSize().z(), "um");
    }
...
std::map<std::string, ElectricFieldReaderModule::FieldData>
    ElectricFieldReaderModule::field_map_;
ElectricFieldReaderModule::FieldData
ElectricFieldReaderModule::get_by_file_name(const std::string &file_name,
                                            Detector &detector) {
  ...
  file >> tmp >> tmp >> tmp >>
      tmp; // ignore temperature, flux, rhe (?) and new_drde (?)
  ...
  for (size_t i = 0; i < xsize * ysize * zsize; ++i) {
    ...
    for (size_t j = 0; j < 3; ++j) {
      ...
      (*field)[xind * ysize * zsize * 3 + yind * zsize * 3 + zind * 3 + j] =
          Units::get(input, "V/cm");
    }
  }
