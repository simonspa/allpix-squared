...
/**
 * Besides binding the message and setting defaults for the configuration, the
 * module copies some configuration variables to
 * local copies to speed up computation.
 */
GenericPropagationModule::GenericPropagationModule(
    Configuration &config, Messenger *messenger,
    std::shared_ptr<Detector> detector)
    : Module(config, detector), messenger_(messenger),
  ...
  messenger_->bindSingle(this, &GenericPropagationModule::deposits_message_,
                         MsgFlags::REQUIRED);
  ...
  config_.setDefault<double>("output_plots_step",
                             config_.get<double>("timestep_max"));
  ...
  if (!config_.get<bool>("propagate_electrons") &&
      !config_.get<bool>("propagate_holes")) {
    throw InvalidValueError(config_, "propagate_electrons",
                            "No charge carriers selected for propagation, "
                            "enable 'propagate_electrons' or "
                            "'propagate_holes'.");
  }
  ...
  // Parameterization variables from
  // https://doi.org/10.1016/0038-1101(77)90054-5 (section 5.2)
...
void GenericPropagationModule::create_output_plots(unsigned int event_num) {
  ...
  if (config_.get<bool>("output_plots_use_equal_scaling", true)) {
    ...
    if (config_.get<bool>("output_plots_use_pixel_units")) {
      minX = centerX -
             model_->getSensorSize().z() / model_->getPixelSize().x() / 2.0;
      maxX = centerX +
             model_->getSensorSize().z() / model_->getPixelSize().x() / 2.0;

      minY = centerY -
             model_->getSensorSize().z() / model_->getPixelSize().y() / 2.0;
      maxY = centerY +
             model_->getSensorSize().z() / model_->getPixelSize().y() / 2.0;
    } else {
  ...
  auto *histogram_frame = new TH3F(
      ("frame_" + getUniqueName() + "_" + std::to_string(event_num)).c_str(),
      "", 10, minX, maxX, 10, minY, maxY, 10,
  ...
  auto canvas = std::make_unique<TCanvas>(
      ("line_plot_" + std::to_string(event_num)).c_str(),
      ("Propagation of charge for event " + std::to_string(event_num)).c_str(),
      1280, 1024);
  canvas->cd();
  canvas->SetTheta(config_.get<float>("output_plots_theta") * 180.0f /
                   ROOT::Math::Pi());
  canvas->SetPhi(config_.get<float>("output_plots_phi") * 180.0f /
                 ROOT::Math::Pi());

  // Draw the frame on the canvas
  histogram_frame->GetXaxis()->SetTitle(
      (std::string("x ") + (config_.get<bool>("output_plots_use_pixel_units")
                                ? "(pixels)"
                                : "(mm)"))
          .c_str());
  histogram_frame->GetYaxis()->SetTitle(
      (std::string("y ") + (config_.get<bool>("output_plots_use_pixel_units")
                                ? "(pixels)"
                                : "(mm)"))
          .c_str());
  ...
  // The vector of unique_pointers is required in order not to delete the
  // objects before the canvas is drawn.
  ...
  for (auto &deposit_points : output_plot_points_) {
    ...
    if (line->GetN() >= 3) {
      EColor plot_color =
          (deposit_points.first.getType() == CarrierType::ELECTRON
               ? EColor::kAzure
               : EColor::kOrange);
      current_color = static_cast<short int>(
          plot_color - 9 + (static_cast<int>(current_color) + 1) % 19);
  ...
  canvas = std::make_unique<TCanvas>(
      ("animation_" + std::to_string(event_num)).c_str(),
      ("Propagation of charge for event " + std::to_string(event_num)).c_str(),
      1280, 1024);
  canvas->cd();

  // Change axis labels if close to zero or PI as they behave different here
  if (std::fabs(config_.get<double>("output_plots_theta") /
                    (ROOT::Math::Pi() / 2.0) -
                std::round(config_.get<double>("output_plots_theta") /
                           (ROOT::Math::Pi() / 2.0))) < 1e-6 ||
      std::fabs(config_.get<double>("output_plots_phi") /
                    (ROOT::Math::Pi() / 2.0) -
                std::round(config_.get<double>("output_plots_phi") /
                           (ROOT::Math::Pi() / 2.0))) < 1e-6) {
  ...
  if (config_.get<bool>("output_animations")) {
    ...
    file_name_contour.push_back(
        createOutputFile("contourX" + std::to_string(event_num) + ".gif"));
    histogram_contour.push_back(new TH2F(
        ("contourX_" + getUniqueName() + "_" + std::to_string(event_num))
            .c_str(),
        "", 100, minY, maxY, 100,
        model_->getSensorCenter().z() - model_->getSensorSize().z() / 2.0,
        model_->getSensorCenter().z() + model_->getSensorSize().z() / 2.0));
    ...
    file_name_contour.push_back(
        createOutputFile("contourY" + std::to_string(event_num) + ".gif"));
    histogram_contour.push_back(new TH2F(
        ("contourY_" + getUniqueName() + "_" + std::to_string(event_num))
            .c_str(),
        "", 100, minX, maxX, 100,
        model_->getSensorCenter().z() - model_->getSensorSize().z() / 2.0,
        model_->getSensorCenter().z() + model_->getSensorSize().z() / 2.0));
    histogram_contour.back()->SetDirectory(getROOTDirectory());
    file_name_contour.push_back(
        createOutputFile("contourZ" + std::to_string(event_num) + ".gif"));
    histogram_contour.push_back(new TH2F(
        ("contourZ_" + getUniqueName() + "_" + std::to_string(event_num))
            .c_str(),
        "", 100, minX, maxX, 100, minY, maxY));
    histogram_contour.back()->SetDirectory(getROOTDirectory());

    // Create file and disable statistics for histogram
    std::string file_name_anim =
        createOutputFile("animation" + std::to_string(event_num) + ".gif");
    ...
    for (int i = 20; i < 100; ++i) {
      ...
      colors[i - 20] =
          new TColor(color_idx, static_cast<float>(i) / 100.0f - 0.2f,
                     static_cast<float>(i) / 100.0f - 0.2f,
                     static_cast<float>(i) / 100.0f - 0.2f);
    }

    // Create animation of moving charges
    auto animation_time = static_cast<unsigned int>(std::round(
        (Units::convert(config_.get<long double>("output_plots_step"), "ms") /
         10.0) *
    ...
    while (point_cnt < tot_point_cnt) {
      ...
      histogram_frame->GetXaxis()->SetTitle(
          (std::string("x ") +
           (config_.get<bool>("output_plots_use_pixel_units") ? "(pixels)"
                                                              : "(mm)"))
              .c_str());
      histogram_frame->GetYaxis()->SetTitle(
          (std::string("y ") +
      ...
      auto time_ns = Units::convert(
          plot_idx * config_.get<long double>("output_plots_step"), "ns");
      ...
      for (auto &deposit_points : output_plot_points_) {
        ...
        auto diff = static_cast<unsigned long>(
            std::round((deposit_points.first.getEventTime() - start_time) /
        ...
        if (config_.get<bool>("output_animations_color_markers")) {
          marker->SetMarkerColor(
              static_cast<Color_t>(colors[initial_z_perc]->GetNumber()));
        }
        marker->SetNextPoint(points[idx].x(), points[idx].y(), points[idx].z());
        marker->Draw();
        markers.push_back(std::move(marker));

        histogram_contour[0]->Fill(points[idx].y(), points[idx].z(),
                                   deposit_points.first.getCharge());
        histogram_contour[1]->Fill(points[idx].x(), points[idx].z(),
                                   deposit_points.first.getCharge());
        histogram_contour[2]->Fill(points[idx].x(), points[idx].y(),
                                   deposit_points.first.getCharge());
        ++point_cnt;
      }
      ...
      if (min_idx_diff != 0) {
        ...
        if (point_cnt < tot_point_cnt - 1) {
          canvas->Print(
              (file_name_anim + "+" + std::to_string(animation_time)).c_str());
        } else {
          canvas->Print((file_name_anim + "++100").c_str());
        }
        ...
        for (size_t i = 0; i < 3; ++i) {
          canvas->Clear();
          canvas->SetTitle(
              (std::string("Contour of charge propagation projected on the ") +
      ...
      LOG_PROGRESS(INFO, getUniqueName() + "_OUTPUT_PLOTS")
          << "Written " << point_cnt << " of " << tot_point_cnt
...
void GenericPropagationModule::init() {
  ...
  // Check for electric field and output warning for slow propagation if not
  // defined
  if (!detector->hasElectricField()) {
    LOG(WARNING) << "This detector does not have an electric field.";
  }

  // For linear fields we can in addition check if the correct carriers are
  // propagated
  if (detector->getElectricFieldType() == ElectricFieldType::LINEAR) {
    auto model = detector_->getModel();
    auto probe_point = ROOT::Math::XYZPoint(
        model->getSensorCenter().x(), model->getSensorCenter().y(),
    ...
    if (direction && !config_.get<bool>("propagate_electrons")) {
      LOG(WARNING) << "Electric field indicates electron collection at "
                      "implants, but electrons are not propagated!";
    }
    if (!direction && !config_.get<bool>("propagate_holes")) {
      LOG(WARNING) << "Electric field indicates hole collection at implants, "
                      "but holes are not propagated!";
    }
  }
  ...
  if (has_magnetic_field_) {
    if (config_.get<bool>("ignore_magnetic_field")) {
      ...
      LOG(WARNING) << "A magnetic field is switched on, but is set to be "
  ...
  if (output_plots_) {
    auto time_bins = static_cast<int>(Units::convert(
        integration_time_ / config_.get<long double>("output_plots_step"),
        "ns"));
    drift_time_histo =
        new TH1D("drift_time_histo", "Drift time;t[ns];charge carriers",
                 time_bins, 0., static_cast<int>(integration_time_));
  }
}
...
void GenericPropagationModule::run(unsigned int event_num) {
  ...
  for (auto &deposit : deposits_message_->getData()) {
    ...
    if ((deposit.getType() == CarrierType::ELECTRON &&
         !config_.get<bool>("propagate_electrons")) ||
        (deposit.getType() == CarrierType::HOLE &&
         !config_.get<bool>("propagate_holes"))) {
    ...
    while (charges_remaining > 0) {
      // Define number of charges to be propagated and remove charges of this
      // step from the total
      ...
      if (output_plots_) {
        ...
        output_plot_points_.emplace_back(
            PropagatedCharge(position, global_position, deposit.getType(),
                             charge_per_step, deposit.getEventTime()),
      ...
      LOG(DEBUG) << " Propagated " << charge_per_step << " to "
                 << Units::display(position, {"mm", "um"}) << " in "
                 << Units::display(prop_pair.second, "ns") << " time";

      // Create a new propagated charge and add it to the list
      auto global_position = detector_->getGlobalPosition(position);
      PropagatedCharge propagated_charge(
          position, global_position, deposit.getType(), charge_per_step,
      ...
      if (output_plots_) {
        drift_time_histo->SetBinContent(
            ...
            drift_time_histo->GetBinContent(
                drift_time_histo->FindBin(prop_pair.second)) +
                charge_per_step);
      }
    }
  ...
  long double average_time =
      total_time / std::max(1u, propagated_charges_count);
  LOG(INFO) << "Propagated " << propagated_charges_count << " charges in "
            << step_count << " steps in average time of "
  ...
  auto propagated_charge_message = std::make_shared<PropagatedChargeMessage>(
      std::move(propagated_charges), detector_);
...
/**
 * Propagation is simulated using a parameterization for the electron mobility.
 * This is used to calculate the electron
 * velocity at every point with help of the electric field map of the detector.
 * An Runge-Kutta integration is applied in
 * multiple steps, adding a random diffusion to the propagating charge every
 * step.
 */
...
GenericPropagationModule::propagate(const ROOT::Math::XYZPoint &pos,
  ...
  // NOTE This function is typically the most frequently executed part of the
  // framework and therefore the bottleneck
  auto carrier_mobility = [&](double efield_mag) {
    ...
    if (type == CarrierType::ELECTRON) {
      ...
      denominator =
          std::pow(1. + std::pow(efield_mag / electron_Ec_, electron_Beta_),
                   1.0 / electron_Beta_);
    } else {
      numerator = hole_Vm_ / hole_Ec_;
      denominator = std::pow(1. + std::pow(efield_mag / hole_Ec_, hole_Beta_),
                             1.0 / hole_Beta_);
    }
  ...
  auto carrier_diffusion = [&](double efield_mag,
                               double timestep) -> Eigen::Vector3d {
  ...
  std::function<Eigen::Vector3d(double, Eigen::Vector3d)>
    ...
    Eigen::Vector3d bfield(magnetic_field_.x(), magnetic_field_.y(),
                           magnetic_field_.z());
    ...
    double hallFactor =
        (type == CarrierType::ELECTRON ? electron_Hall_ : hole_Hall_);
    term1 = static_cast<int>(type) * mob * hallFactor * exb;

    Eigen::Vector3d term2 =
        mob * mob * hallFactor * hallFactor * efield.dot(bfield) * bfield;

    auto rnorm = 1 + mob * mob * hallFactor * hallFactor * bfield.dot(bfield);
    return static_cast<int>(type) * mob * (efield + term1 + term2) / rnorm;
  };

  // Create the runge kutta solver with an RKF5 tableau, using different
  // velocity calculators depending on the magnetic
  // field
  ...
  size_t next_idx = 0;
  while (
      detector_->isWithinSensor(static_cast<ROOT::Math::XYZPoint>(position)) &&
      runge_kutta.getTime() < integration_time_) {
    // Update output plots if necessary (depending on the plot step)
    if (output_plots_) {
      auto time_idx =
          static_cast<size_t>(runge_kutta.getTime() / output_plots_step_);
      while (next_idx <= time_idx) {
        output_plot_points_.back().second.push_back(
    ...
    // Get electric field at current position and fall back to empty field if it
    // does not exist
    auto efield = detector_->getElectricField(
        static_cast<ROOT::Math::XYZPoint>(position));
    ...
    if (std::fabs(model_->getSensorSize().z() / 2.0 - position.z()) <
        2 * step.value.z()) {
      timestep *= 0.75;
    } else {
  ...
  if (!detector_->isWithinSensor(static_cast<ROOT::Math::XYZPoint>(position))) {
    ...
    if (position.z() > 0 &&
        detector_->isWithinSensor(
            static_cast<ROOT::Math::XYZPoint>(check_position))) {
      // Carrier left sensor on the side of the pixel grid, interpolate end
      // point on surface
      auto z_cur_border =
          std::fabs(position.z() - model_->getSensorSize().z() / 2.0);
      auto z_last_border =
          std::fabs(model_->getSensorSize().z() / 2.0 - last_position.z());
      auto z_total = z_cur_border + z_last_border;
      position = (z_last_border / z_total) * position +
                 (z_cur_border / z_total) * last_position;
      time = (z_last_border / z_total) * time +
             (z_cur_border / z_total) * last_time;
    } else {
      // Carrier left sensor on any order border, use last position inside
      // instead
...
void GenericPropagationModule::finalize() {
  long double average_time =
      total_time_ / std::max(1u, total_propagated_charges_);
  LOG(INFO) << "Propagated total of " << total_propagated_charges_
            << " charges in " << total_steps_ << " steps in average time of "
            << Units::display(average_time, "ns");
