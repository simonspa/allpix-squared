...
/**
 * Includes the particle source point to the geometry using \ref
 * GeometryManager::addPoint.
 */
DepositionGeant4Module::DepositionGeant4Module(Configuration &config,
                                               Messenger *messenger,
                                               GeometryManager *geo_manager)
    : Module(config), messenger_(messenger), geo_manager_(geo_manager),
      last_event_num_(1), run_manager_g4_(nullptr) {
  // Create user limits for maximum step length in the sensor
  user_limits_ = std::make_unique<G4UserLimits>(
      config_.get<double>("max_step_length", Units::get(1.0, "um")));
...
/**
 * Module depends on \ref GeometryBuilderGeant4Module loaded first, because it
 * owns the pointer to the Geant4 run manager.
 */
void DepositionGeant4Module::init() {
  ...
  if (run_manager_g4_ == nullptr) {
    throw ModuleError("Cannot deposit charges using Geant4 without a Geant4 "
                      "geometry builder");
  }
  ...
  if (config_.get<bool>("enable_pai", false)) {
    ...
    for (auto &detector : geo_manager_->getDetectors()) {
      // Get logical volume
      auto logical_volume =
          detector->getExternalObject<G4LogicalVolume>("sensor_log");
      if (logical_volume == nullptr) {
        throw ModuleError("Detector " + detector->getName() +
      ...
      std::transform(lcase_model.begin(), lcase_model.end(),
                     lcase_model.begin(), ::tolower);
      if (lcase_model == "pai") {
        pai_model = "PAI";
      } else if (lcase_model == "paiphoton") {
        pai_model = "PAIphoton";
      } else {
        throw InvalidValueError(config_, "pai_model",
                                "model has to be either 'pai' or 'paiphoton'");
      }

      ui_g4->ApplyCommand("/process/em/AddPAIRegion all " + region->GetName() +
                          " " + pai_model);
    }
  }
  ...
  G4VModularPhysicsList *physicsList = physListFactory.GetReferencePhysList(
      config_.get<std::string>("physics_list"));
  if (physicsList == nullptr) {
    ...
    LOG(INFO) << "Using G4 physics list \""
              << config_.get<std::string>("physics_list") << "\"";
  }
  ...
  if (config_.has("range_cut")) {
    ...
    LOG(INFO) << "Setting configured G4 production cut to "
              << Units::display(production_cut, {"mm", "um"});
  } else {
    // Define the production cut as one fifth of the minimum size (thickness,
    // pitch) among the detectors
    ...
    for (auto &detector : geo_manager_->getDetectors()) {
      ...
      min_size =
          std::min({min_size, model->getPixelSize().x(),
                    model->getPixelSize().y(), model->getSensorSize().z()});
      if (min_size != prev_min_size) {
        min_detector = detector->getName();
      }
    }
    production_cut = min_size / 5;
    LOG(INFO) << "Setting G4 production cut to "
              << Units::display(production_cut, {"mm", "um"})
  ...
  // User hook to store additional information at track initialization and
  // termination as well as custom track ids
  ...
  if (geo_manager_->hasMagneticField()) {
    MagneticFieldType magnetic_field_type_ =
        geo_manager_->getMagneticFieldType();

    if (magnetic_field_type_ == MagneticFieldType::CONSTANT) {
      ROOT::Math::XYZVector b_field =
          geo_manager_->getMagneticField(ROOT::Math::XYZPoint(0., 0., 0.));
      G4MagneticField *magField = new G4UniformMagField(
          G4ThreeVector(b_field.x(), b_field.y(), b_field.z()));
      ...
      throw ModuleError("Magnetic field enabled, but not constant. This can't "
                        "be handled by this module yet.");
    }
  }

  // Get the creation energy for charge (default is silicon electron hole pair
  // energy)
  auto charge_creation_energy =
      config_.get<double>("charge_creation_energy", Units::get(3.64, "eV"));

  // Loop through all detectors and set the sensitive detector action that
  // handles the particle passage
  bool useful_deposition = false;
  for (auto &detector : geo_manager_->getDetectors()) {
    // Do not add sensitive detector for detectors that have no listeners for
    // the deposited charges
    // FIXME Probably the MCParticle has to be checked as well
    if (!messenger_->hasReceiver(
            this, std::make_shared<DepositedChargeMessage>(
                      std::vector<DepositedCharge>(), detector))) {
    ...
    auto sensitive_detector_action = new SensitiveDetectorActionG4(
        this, detector, messenger_, track_info_manager_.get(),
    ...
    if (config_.get<bool>("output_plots")) {
      ...
      int maximum = static_cast<int>(
          Units::convert(config_.get<int>("output_plots_scale"), "ke"));
      int nbins = 5 * maximum;

      // Create histograms if needed
      std::string plot_name =
          "deposited_charge_" + sensitive_detector_action->getName();
      charge_per_event_[sensitive_detector_action->getName()] =
          new TH1D(plot_name.c_str(),
  ...
  if (!useful_deposition) {
    LOG(ERROR)
        << "Not a single listener for deposited charges, module is useless!";
  }
  ...
  for (int i = 0; i < G4_NUM_SEEDS; ++i) {
    seed_command +=
        std::to_string(static_cast<uint32_t>(getRandomSeed() % INT_MAX));
    if (i != G4_NUM_SEEDS - 1) {
      seed_command += " ";
    }
  }
...
void DepositionGeant4Module::run(unsigned int event_num) {
  ...
  for (auto &sensor : sensors_) {
    ...
    if (config_.get<bool>("output_plots")) {
      double charge = static_cast<double>(
          Units::convert(sensor->getDepositedCharge(), "ke"));
...
void DepositionGeant4Module::finalize() {
  ...
  if (!sensors_.empty() && total_charges > 0 && last_event_num_ > 0) {
    ...
    LOG(INFO) << "Deposited total of " << total_charges << " charges in "
              << sensors_.size() << " sensor(s) (average of " << average_charge
              << " per sensor for every event)";
  } else {
