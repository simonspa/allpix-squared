...
std::unique_ptr<TrackInfoG4>
TrackInfoManager::makeTrackInfo(const G4Track *const track) {
  auto custom_id = counter_++;
  auto G4ParentID = track->GetParentID();
  auto parent_track_id =
      G4ParentID == 0 ? G4ParentID : g4_to_custom_id_.at(G4ParentID);
...
void TrackInfoManager::setTrackInfoToBeStored(int track_id) {
  auto element = std::find(to_store_track_ids_.begin(),
                           to_store_track_ids_.end(), track_id);
  // If track id is not present we add it, otherwise skip as we only need each
  // track once
...
void TrackInfoManager::storeTrackInfo(
...
void TrackInfoManager::dispatchMessage(Module *module, Messenger *messenger) {
  setAllTrackParents();
  IFLOG(DEBUG) {
    LOG(DEBUG) << "Dispatching " << stored_tracks_.size()
               << " MCTrack(s) from TrackInfoManager::dispatchMessage()";
    for (auto const &mc_track : stored_tracks_) {
      LOG(DEBUG) << "MCTrack originates at: "
                 << Units::display(mc_track.getStartPoint(), {"mm", "um"})
                 << " and terminates at: "
                 << Units::display(mc_track.getEndPoint(), {"mm", "um"});
    }
  }
  auto mc_track_message =
      std::make_shared<MCTrackMessage>(std::move(stored_tracks_));
...
void TrackInfoManager::createMCTracks() {
  for (auto &track_info : stored_track_infos_) {
    stored_tracks_.emplace_back(
        track_info->getStartPoint(), track_info->getEndPoint(),
        track_info->getOriginatingVolumeName(),
        track_info->getCreationProcessName(),
        track_info->getCreationProcessType(), track_info->getParticleID(),
        track_info->getKineticEnergyInitial(),
        track_info->getKineticEnergyFinal(),
        track_info->getTotalEnergyInitial(), track_info->getTotalEnergyFinal());
