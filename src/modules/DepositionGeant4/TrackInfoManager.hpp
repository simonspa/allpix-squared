/**
 * @file
 * @brief The TrackInfoManager class, contains a factory method for TrackInfoG4
 * to be used in AP2 as well as handling and
...
namespace allpix {
/**
 * @brief The TrackInfoManager is a factory for TrackInfoG4 objects and manages
 * MCTracks within AP2
 */
...
public:
   ...
   *
   * This factory method will take care that it will only assign every track id
   * once for this
   ...
   *
   * It will be stored if it was registered to be stored (@see
   * #setTrackInfoToBeStored), otherwise deleted
   */
   ...
   *
   * The track itself will have to be provided via @see storeTrackInfo once
   * finished
   */
   ...
   *
   * This will reset the track id counter, the tracks which are registered to be
   * stored and the already stored tracks,
   * regardless if they have been dispatched or not. Make sure to call @see
   * #dispatchMesseges before if the tracks
   ...
   * @warning Must only be called once Geant4 finished stepping through all the
   * G4Track objects
   * guranteed
   */
  void createMCTracks();
  ...
  /**
   * @brief Returns a pointer to the MCTrack object in the #stored_tracks_ or a
   * nullptr if not found
   * @param track_id The id of the track for which to retrieve the pointer
   * @return Const pointer to the MCTrack object or a nullptr if track_id is not
   * found
   * @warning Results are invalidated by any reallocation iof the internal
   * #stored_tracks_ vector
   */
  MCTrack const *findMCTrack(int track_id) const;

private:
  /**
   * @brief Will internally set all the parent-child relations between stored
   * tracks
   * @warning This must only be called once all the tracks are created (@see
   * #createMCTracks) and no reallocation of
