...
SensitiveDetectorActionG4::SensitiveDetectorActionG4(
    Module *module, const std::shared_ptr<Detector> &detector, Messenger *msg,
    TrackInfoManager *track_info_manager, double charge_creation_energy)
    : G4VSensitiveDetector("SensitiveDetector_" + detector->getName()),
      module_(module), detector_(detector), messenger_(msg),
      track_info_manager_(track_info_manager),
      charge_creation_energy_(charge_creation_energy) {
...
G4bool SensitiveDetectorActionG4::ProcessHits(G4Step *step,
                                              G4TouchableHistory *) {
  ...
  G4ThreeVector mid_pos =
      (preStepPoint->GetPosition() + postStepPoint->GetPosition()) / 2;
  double mid_time =
      (preStepPoint->GetGlobalTime() + postStepPoint->GetGlobalTime()) / 2;

  // Calculate the charge deposit at a local position
  auto deposit_position =
      detector_->getLocalPosition(static_cast<ROOT::Math::XYZPoint>(mid_pos));
  auto deposit_position_g4 =
      theTouchable->GetHistory()->GetTopTransform().TransformPoint(mid_pos);
  auto charge = static_cast<unsigned int>(edep / charge_creation_energy_);

  auto deposit_position_g4loc = ROOT::Math::XYZPoint(
      deposit_position_g4.x() + detector_->getModel()->getSensorCenter().x(),
      deposit_position_g4.y() + detector_->getModel()->getSensorCenter().y(),
      deposit_position_g4.z() + detector_->getModel()->getSensorCenter().z());

  const auto userTrackInfo =
      dynamic_cast<TrackInfoG4 *>(step->GetTrack()->GetUserInformation());
  ...
  if (track_begin_.find(trackID) == track_begin_.end()) {
    ...
    auto start_position = detector_->getLocalPosition(
        static_cast<ROOT::Math::XYZPoint>(preStepPoint->GetPosition()));
    track_begin_.emplace(trackID, start_position);
    track_parents_.emplace(trackID, parentTrackID);
    track_pdg_.emplace(trackID,
                       step->GetTrack()->GetDynamicParticle()->GetPDGcode());
  }

  // Update current end point with the current last step
  auto end_position = detector_->getLocalPosition(
      static_cast<ROOT::Math::XYZPoint>(postStepPoint->GetPosition()));
  ...
  // Deposit electron
  deposits_.emplace_back(deposit_position, global_deposit_position,
                         CarrierType::ELECTRON, charge, mid_time);
  deposit_to_id_.push_back(trackID);

  // Deposit hole
  deposits_.emplace_back(deposit_position, global_deposit_position,
                         CarrierType::HOLE, charge, mid_time);
  deposit_to_id_.push_back(trackID);

  LOG(DEBUG) << "Created deposit of " << charge << " charges at "
             << Units::display(mid_pos, {"mm", "um"}) << " locally on "
             << Units::display(deposit_position, {"mm", "um"}) << " in "
             << detector_->getName() << " after "
             << Units::display(mid_time, {"ns", "ps"});

  LOG(DEBUG) << "Geant4 transformation to local: "
             << Units::display(deposit_position_g4loc, {"mm", "um"});
  if ((deposit_position_g4loc - deposit_position).mag2() > 0.001) {
    LOG(ERROR) << "Difference G4 to internal: "
               << Units::display((deposit_position_g4loc - deposit_position),
                                 {"mm", "um"});
  }
  return true;
}
...
void SensitiveDetectorActionG4::dispatchMessages() {
  ...
  for (auto &track_id_point : track_begin_) {
    ...
    LOG(DEBUG) << "Found MC particle " << pdg_code << " crossing detector from "
               << Units::display(local_begin, {"mm", "um"}) << " to "
               << Units::display(local_end, {"mm", "um"})
               << " (local coordinates)";
  }
  ...
  for (auto &track_parent : track_parents_) {
    ...
    if (id_to_particle_.find(parent_id) == id_to_particle_.end()) {
      ...
      // FIXME: Geant4 does not allow for an easy way retrieve the whole
      // hierarchy
      continue;
    }
  ...
  auto mc_particle_message =
      std::make_shared<MCParticleMessage>(std::move(mc_particles), detector_);
  ...
  if (!deposits_.empty()) {
    ...
    LOG(INFO) << "Deposited " << charges << " charges in sensor of detector "
              << detector_->getName();
    ...
    for (size_t i = 0; i < deposits_.size(); ++i) {
      ...
      deposits_.at(i).setMCParticle(
          &mc_particle_message->getData().at(id_to_particle_.at(track_id)));
    }

    // Create a new charge deposit message
    auto deposit_message = std::make_shared<DepositedChargeMessage>(
        std::move(deposits_), detector_);
