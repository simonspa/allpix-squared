...
ProjectionPropagationModule::ProjectionPropagationModule(
  ...
  messenger_->bindSingle(this, &ProjectionPropagationModule::deposits_message_,
                         MsgFlags::REQUIRED);
  ...
  if (config_.get<bool>("propagate_holes")) {
    ...
    LOG(INFO) << "Holes are chosen for propagation. Electrons are therefore "
                 "not propagated.";
  } else {
...
void ProjectionPropagationModule::init() {
  if (detector_->getElectricFieldType() != ElectricFieldType::LINEAR) {
    throw ModuleError(
        "This module should only be used with linear electric fields.");
  }

  if (detector_->hasMagneticField() &&
      !config_.get<bool>("ignore_magnetic_field")) {
    throw ModuleError("This module should not be used with magnetic fields. "
                      "Add the option 'ignore_magnetic_field' to "
                      "the configuration if you would like to continue.");
  } else if (detector_->hasMagneticField() &&
             config_.get<bool>("ignore_magnetic_field")) {
    LOG(WARNING) << "A magnetic field is switched on, but is set to be ignored "
                    "for this module.";
  }
  ...
  if (output_plots_) {
    ...
    drift_time_histo =
        new TH1D("drift_time_histo", "Drift time;t[ns];particles", 75, 0., 25.);
  }
}
...
void ProjectionPropagationModule::run(unsigned int) {
  ...
  for (auto &deposit : deposits_message_->getData()) {
    ...
    LOG(DEBUG) << "Set of " << deposit.getCharge() << " charge carriers ("
               << type << ") on " << Units::display(position, {"mm", "um"});
    ...
    // Get the electric field at the position of the deposited charge and the
    // top of the sensor:
    auto efield = detector_->getElectricField(position);
    double efield_mag = std::sqrt(efield.Mag2());
    auto efield_top = detector_->getElectricField(
        ROOT::Math::XYZPoint(0., 0., model->getSensorSize().z() / 2.));
    double efield_mag_top = std::sqrt(efield_top.Mag2());

    LOG(TRACE) << "Electric field at carrier position / top of the sensor: "
               << Units::display(efield_mag_top, "V/cm") << " , "
               << Units::display(efield_mag, "V/cm");

    slope_efield_ = (efield_mag_top - efield_mag) /
                    (model->getSensorSize().z() / 2. - position.z());
    ...
    auto calc_drift_time = [&]() {
      ...
      double zero_mobility =
          (type == CarrierType::ELECTRON ? electron_Vm_ / electron_Ec_
                                         : hole_Vm_ / hole_Ec_);
    ...
    if (efield_mag < std::numeric_limits<double>::epsilon()) {
      LOG(TRACE) << "Electric field is zero at "
    ...
    double diffusion_constant =
        boltzmann_kT_ *
        (carrier_mobility(efield_mag) + carrier_mobility(efield_mag_top)) / 2.;
    ...
    if (output_plots_) {
      drift_time_histo->SetBinContent(
          drift_time_histo->FindBin(drift_time),
    ...
    LOG(TRACE) << "Diffusion width is "
               << Units::display(diffusion_std_dev, "um");
    ...
    while (charges_remaining > 0) {
      ...
      auto projected_position = ROOT::Math::XYZPoint(
          position.x() + diffusion_x, position.y() + diffusion_y,
          model->getSensorSize().z() / 2.);

      // Only add if within sensor volume:
      auto local_position =
          ROOT::Math::XYZPoint(projected_position.x(), projected_position.y(),
                               model->getSensorSize().z() / 2.);
      ...
      propagated_charges.emplace_back(projected_position, global_position,
                                      deposit.getType(), charge_per_step,
                                      deposit.getEventTime(), &deposit);
  ...
  LOG(DEBUG) << "Total count of propagated charge carriers: "
             << propagated_charges.size();
