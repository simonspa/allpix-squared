...
ROOTObjectWriterModule::ROOTObjectWriterModule(Configuration &config,
                                               Messenger *messenger,
...
void ROOTObjectWriterModule::init() {
  ...
  output_file_name_ = createOutputFile(
      allpix::add_file_extension(config_.get<std::string>("file_name", "data"),
                                 "root"),
      true);
...
void ROOTObjectWriterModule::receive(std::shared_ptr<BaseMessage> message,
  ...
  try {
    ...
    LOG(TRACE) << "ROOT object writer received "
               << allpix::demangle(typeid(*inst).name()) << name_str;
    ...
    if (!object_array.empty()) {
      ...
      // Create a new branch of the correct type if this message was not
      // received before
      ...
      if (write_list_.find(index_tuple) == write_list_.end()) {
        ...
        if ((!include_.empty() &&
             include_.find(class_name) == include_.end()) ||
            (!exclude_.empty() &&
             exclude_.find(class_name) != exclude_.end())) {
          LOG(TRACE)
              << "ROOT object writer ignored message with object "
              << allpix::demangle(typeid(*inst).name())
        ...
        if (trees_.find(class_name) == trees_.end()) {
          ...
          trees_.emplace(class_name,
                         std::make_unique<TTree>(
                             class_name.c_str(),
                             (std::string("Tree of ") + class_name).c_str()));
        }

        std::string branch_name =
            detector_name.empty() ? "global" : detector_name;
        if (!message_name.empty()) {
          branch_name += "_";
    ...
    const BaseMessage *inst = message.get();
    LOG(WARNING) << "ROOT object writer cannot process message of type"
                 << allpix::demangle(typeid(*inst).name()) << " with name "
                 << message_name;
  }
}
...
void ROOTObjectWriterModule::run(unsigned int) {
  ...
  // Clear the messages we have to keep because they contain the internal
  // pointers
...
void ROOTObjectWriterModule::finalize() {
  ...
  // Print statistics
  LOG(STATUS) << "Wrote " << write_cnt_ << " objects to " << branch_count
              << " branches in file:" << std::endl
              << output_file_name_;
}
