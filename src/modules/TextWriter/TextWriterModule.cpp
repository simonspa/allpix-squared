...
TextWriterModule::TextWriterModule(Configuration &config, Messenger *messenger,
                                   GeometryManager *)
    : Module(config) {
...
void TextWriterModule::receive(std::shared_ptr<BaseMessage> message,
  ...
  try {
    ...
    LOG(TRACE) << "Text writer received "
    ...
    if (!object_array.empty()) {
      ...
      if ((!include_.empty() && include_.find(class_name) == include_.end()) ||
        ...
        LOG(TRACE)
            << "Text writer ignored message with object "
    ...
    const BaseMessage *inst = message.get();
    LOG(WARNING) << "Text writer cannot process message of type"
...
void TextWriterModule::finalize() {
  ...
  *output_file_ << "# " << write_cnt_ << " objects from " << msg_cnt_
                << " messages" << std::endl;

  // Print statistics
  LOG(STATUS) << "Wrote " << write_cnt_ << " objects from " << msg_cnt_
              << " messages to file:" << std::endl
              << output_file_name_;
}
