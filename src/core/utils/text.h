...
/**
 * @brief Internal helper method for checking and trimming conversions from
 * string
 ...
 * @throws std::invalid_argument If no closing quotation mark as last character
 * after an opening quotation mark
 * @throws std::invalid_argument If string without enclosing quotation marks,
 * but more data after whitespace is found
 */
 ...
 * whitespace.
 */
inline std::string to_string_impl(const std::string &inp, empty_tag) {
  return '"' + inp + '"';
}
inline std::string to_string_impl(const char *inp, empty_tag) {
  return '"' + std::string(inp) + '"';
}
inline std::string to_string_impl(char *inp, empty_tag) {
  return '"' + std::string(inp) + '"';
}
///@}
 ...
 * @return List of all the substrings with all empty substrings ignored (thus
 * removed)
 */
template <typename T>
std::vector<T> split(std::string str, const std::string &delims = " \t,");
} // namespace allpix
