...
public:
   ...
   */
  static std::string display(UnitType input,
  ...
  /**
   * @brief Return value for display in the best of all the provided units for
   * all vector elements
   * @param input Vector of values in the base unit system
   * @param units Name of the possible output units
   * @return Vector of values between parentheses with best display units for
   * each element
   * @note Works for all vector types that can be converted to a string using
   * \ref StringConversions "the string
   * utilities".
   */
  template <typename T>
  static std::string display(T input, std::initializer_list<std::string> units);
