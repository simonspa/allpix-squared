...
MCTrack::MCTrack(ROOT::Math::XYZPoint start_point,
                 ROOT::Math::XYZPoint end_point, std::string g4_volume,
                 std::string g4_prod_process_name, int g4_prod_process_type,
                 int particle_id, double initial_kin_E, double final_kin_E,
                 double initial_tot_E, double final_tot_E)
    : start_point_(std::move(start_point)), end_point_(std::move(end_point)),
      origin_g4_vol_name_(std::move(g4_volume)),
      origin_g4_process_name_(std::move(g4_prod_process_name)),
      origin_g4_process_type_(g4_prod_process_type), particle_id_(particle_id),
      initial_kin_E_(initial_kin_E), final_kin_E_(final_kin_E),
...
int MCTrack::getNumberOfSteps() const { return n_steps_; }
...
void MCTrack::print(std::ostream &out) const {
  ...
  static const size_t largest_output =
      2 * big_gap + 2 * med_gap + 2 * small_gap;
  ...
  out << '\n'
      ...
      << particle_id_ << '\n'
      << std::left << std::setw(big_gap) << "Production process: " << std::right
      << std::setw(small_gap) << origin_g4_process_name_
      << " (G4 process type: " << origin_g4_process_type_ << ")\n"
      << std::left << std::setw(big_gap)
      << "Production in G4Volume: " << std::right << std::setw(small_gap)
      << origin_g4_vol_name_ << '\n'
      << std::left << std::setw(big_gap) << "Initial position:" << std::right
      << std::setw(med_gap) << start_point_.X() << " mm |" << std::setw(med_gap)
      << start_point_.Y() << " mm |" << std::setw(med_gap) << start_point_.Z()
      << " mm\n"
      << std::left << std::setw(big_gap) << "Final position:" << std::right
      << std::setw(med_gap) << end_point_.X() << " mm |" << std::setw(med_gap)
      << end_point_.Y() << " mm |" << std::setw(med_gap) << end_point_.Z()
      << " mm\n"
      << std::left << std::setw(big_gap)
      << "Initial kinetic energy: " << std::right << std::setw(med_gap)
      << initial_kin_E_ << std::setw(small_gap) << " MeV | " << std::left
      << std::setw(big_gap) << "Final kinetic energy: " << std::right
      << std::setw(med_gap) << final_kin_E_ << std::setw(small_gap)
      << " MeV   \n"
      << std::left << std::setw(big_gap)
      << "Initial total energy: " << std::right << std::setw(med_gap)
      << initial_tot_E_ << std::setw(small_gap) << " MeV | " << std::left
      << std::setw(big_gap) << "Final total energy: " << std::right
      << std::setw(med_gap) << final_tot_E_ << std::setw(small_gap)
      << " MeV   \n";
  if (parent != nullptr) {
