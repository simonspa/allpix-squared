...
mesh_converter::read_grid(const std::string &file_name, bool mesh_tree) {
  ...
  while (!file.eof()) {
    ...
    if (line.find('=') != std::string::npos) {
      ...
      if (std::regex_match(line, base_match, base_regex) &&
        ...
        if (main_section == DFSection::INFO) {
          if (key == "dimension" &&
              (std::stoul(value) != 3 && std::stoul(value) != 2)) {
            main_section = DFSection::IGNORED;
          }
          if (key == "dimension" &&
              (std::stoul(value) == 3 || std::stoul(value) == 2)) {
    ...
    case DFSection::EDGES: {
      ...
      while (sstr >> edge.first >> edge.second) {
        if (edge.first >= vertices.size() || edge.second >= vertices.size()) {
          throw std::runtime_error(
              "vertex index is higher than number of vertices");
        }
    ...
    case DFSection::ELEMENTS: {
      ...
      case 8: /* brick */
        ...
        break;
      default:
        throw std::runtime_error("element type " + std::to_string(k) +
                                 " is not supported");
      }
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        if (size == 3 || size == 2) {
          if (element_idx >= static_cast<long>(edges.size())) {
            throw std::runtime_error(
                "edge index is higher than number of faces");
          }
        ...
        if (size == 4) {
          if (element_idx >= static_cast<long>(faces.size())) {
            throw std::runtime_error(
                "face index is higher than number of faces");
          }
    ...
    case DFSection::REGION: {
      ...
      while (sstr >> elem_idx) {
        if (elem_idx >= elements.size()) {
          throw std::runtime_error(
              "element index is higher than number of elements");
        }

        regions_vertices[region].insert(regions_vertices[region].end(),
                                        elements[elem_idx].begin(),
    ...
    if (line.find('{') != std::string::npos) {
      ...
      if (std::regex_match(line, base_match, base_regex) &&
        ...
        if (header_string == "Dataset") {
          ...
          LOG(DEBUG) << "Opening value section with " << header_data
                     << " entries";
    ...
    if (line.find('}') != std::string::npos) {
      ...
      if (main_section == DFSection::ELECTROSTATIC_POTENTIAL &&
        ...
        if (data_count != region_electric_field_num.size()) {
          throw std::runtime_error(
              "incorrect number of electrostatic potential points");
        }
