  ...
  while ((entry = (TKey *)next())) {
    ...
    if (cl->InheritsFrom("TDirectoryFile")) {
      ...
      if (n != std::string::npos) {
        ...
        str << "name = \"" << key.substr(n + 1).substr(0, m) << "\""
            << std::endl;
        named_module = true;
      } else {
        str << "[" << key << "]" << std::endl;
      ...
      // If this is the detectors file, cache its name and add the current
      ...
      ROOT::Math::Rotation3D rot =
          (*(ROOT::Math::Rotation3D *)entry->ReadObj()).Inverse();
      ROOT::Math::RotationZYX angles = ROOT::Math::RotationZYX(rot);
      str << "orientation_type = \"xyz\"" << std::endl;
      str << key << " = " << (-angles.Psi()) << "rad " << (-angles.Theta())
          << "rad " << (-angles.Phi()) << "rad" << std::endl;
    } else if (cl->InheritsFrom("ROOT::Math::PositionVector3D<ROOT::Math::"
                                "Cartesian3D<double>,ROOT::Math::"
      ...
      ROOT::Math::PositionVector3D<ROOT::Math::Cartesian3D<double>,
          ...
          position =
              (*(ROOT::Math::PositionVector3D<
                  ...
                  ROOT::Math::DefaultCoordinateSystemTag> *)entry->ReadObj());
      str << key << " = " << position.x() << "mm " << position.y() << "mm "
          << position.z() << "mm" << std::endl;
    } else {
      std::cout << "Could not deduce parameter type of \"" << key
                << "\": " << entry->GetClassName() << std::endl;
    }
  }
...
void recoverConfiguration(std::string data_file, std::string config_file_name) {
  ...
  if (config != nullptr) {
    std::cout << "Found TDirectory containing the configuration keys."
              << std::endl;
    ...
    if (config_file.good()) {
      config_file << str.str();
      std::cout << "Wrote configuration to: \"" << config_file_name << "\""
                << std::endl;
    }
  } else {
  ...
  if (detectors != nullptr) {
    cout << "Found TDirectory containing the detector configuration."
         << std::endl;
    ...
    if (detectors_file.empty()) {
      ...
      cout << "Using default name for detectors file - you might need to "
              "adjust the parameter.";
    } else {
      // Clean string from quotation marks:
      if (detectors_file.front() == '"') {
        detectors_file.erase(0, 1); // erase the first character
        detectors_file.erase(detectors_file.size() -
                             1); // erase the last character
      }
    }

    detectors_file = path + "/" + detectors_file;
    std::ofstream det_file(detectors_file,
                           std::ios_base::out | std::ios_base::trunc);
    if (det_file.good()) {
      det_file << str.str();
      std::cout << "Wrote detector setup to: \"" << detectors_file << "\""
                << std::endl;
    }
  } else {
    cout << "Could not find TDirectory with detector configuration."
         << std::endl;
  }
  ...
  if (models != nullptr) {
    cout << "Found TDirectory containing the model configurations."
         << std::endl;
    ...
    while ((entry = (TKey *)next())) {
      ...
      if (cl->InheritsFrom("TDirectoryFile")) {
        std::string model_parameters =
            listkeys((TDirectoryFile *)entry->ReadObj()).str();
        std::string model_file = path + "/" + key + ".conf";
        std::ofstream mod_file(model_file,
                               std::ios_base::out | std::ios_base::trunc);
        if (mod_file.good()) {
          // Remove numbering fro support layer headers
          std::regex support_header("\\[support_[0-9]+\\]");
          mod_file << std::regex_replace(model_parameters, support_header,
                                         "[support]");
          std::cout << "Wrote model \"" << key << "\" to: \"" << model_file
                    << "\"" << std::endl;
        }
      } else {
